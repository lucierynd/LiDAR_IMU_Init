...
void Preprocess::process_cut_frame_livox(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg,
    ...
    double last_frame_end_time = stamp_to_sec(msg->header.stamp) * 1000.0;
    uint valid_num = 0;
    uint cut_num = 0;
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        valid_num++;
        //Compute new offset time of each point: ms
        pl_surf[i].curvature += stamp_to_sec(msg->header.stamp) * 1000.0 - last_frame_end_time;
...
void Preprocess::velodyne_handler_kitti(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {
    pl_surf.clear();
    pl_full.clear();
